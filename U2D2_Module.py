""" Auteur: Luca Freund
    ...
    Modif V6:
    Modification Post coupe d europe pour l'optimisation
"""
...
BAUDRATE                = 4_000_000 #Vitesse de Communication
DEVICENAME              = 'COM4'    #Port ou le U2D2 est brancher (PC="COM4"//Raspberry="/dev/ttyUSB0")
...
ADDR_PRESENT_VELOCITY       = 128   #Adresse de la vitesse actuelle du servo
...
class Dxl():
    def __init__(self):
        ...
        try:
            ...
            print("RESOLVE HINT --> The device name is incorrect, or the cable is not connected")
            quit()   
    ...
    def ProfileAcceleration(self,ID, ProfileAcceleration):
        #Definit le mode de d'acceleration
        self.dxl_comm_result, self.dxl_error = self.packetHandler.write4ByteTxRx(self.portHandler, ID, PROFILE_ACCELERATION, ProfileAcceleration)
        if self.dxl_comm_result != COMM_SUCCESS:
            print("%s" % self.packetHandler.getTxRxResult(self.dxl_comm_result))
            self.Erreur = True
        elif self.dxl_error != 0:
    ...
    def VitesseActuel(self, ID):
        # Vitesse actuel du servo
        self.Vitesse, self.dxl_comm_result, self.dxl_error = self.packetHandler.read4ByteTxRx(self.portHandler, ID, ADDR_PRESENT_VELOCITY)
        
        if self.Vitesse > 2047:
           self.Vitesse = (2047 - self.Vitesse) *-1
        return self.Vitesse